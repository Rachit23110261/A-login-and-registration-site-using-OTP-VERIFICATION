...
export const OTPVerifier = () => {
  ...
  const { storeTokenInLS, email ,setIsLoading,} = useAuth();
  const [resendDisabled, setResendDisabled] = useState(false);
  const [resendTimer, setResendTimer] = useState(10);

  const URL = `http://localhost:5000/api/auth/otpverify`;
  ...
  const sendOTP = async (email) => {
    try {
      const response = await fetch('http://localhost:5000/api/auth/otpsend', {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error('Please check your phone address')
      }
    } catch (error) {
